...
let checkDogs = function (dogsJulia, dogsKate) {
  let juliaCorrect = [...dogsJulia];
  juliaCorrect.splice(0, 1);
  juliaCorrect.splice(-2);
  let corrected = juliaCorrect.concat(dogsKate);
  corrected.forEach(function (mov, i) {
    console.log(
      `Dog number ${i + 1} is ${
        mov >= 3 ? `an adult, and is ${mov} years old` : `still a puppy 🐶`
      }`
    );
  });
};
...
let calcAverageHumanAge = function (ages) {
  ...
  let adults = humanAges.filter((age) => {
    return age >= 18;
  });
  let average = adults.reduce((acc, cur, i, arr) => {
...
let calcAverageHumanAge1 = (ages) =>
  ages
    .map((age) => (age <= 2 ? 2 * age : 16 + age * 4))
    .filter((age) => age >= 18)
    .reduce((acc, cur, i, arr) => acc + cur / arr.length, 0);

let avg1 = calcAverageHumanAge1([5, 2, 4, 1, 15, 8, 3]);
let avg2 = calcAverageHumanAge1([16, 6, 10, 5, 6, 1, 4]);
// console.log(avg1);
// console.log(avg2);
...
// console.log(
//   `${ownersEatTooMuch.reduce((str, cur, i, arr) => {
//     return i !== arr.length - 1 ? `${str} ${cur} and` : `${str} ${cur}`;
//   }, "")} 's eat too much!`.trim()
// );
